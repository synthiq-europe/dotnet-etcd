...
[Trait("Category", "Unit")]
public sealed class EtcdKeyVersionTests
{
    [Fact]
    public void DefaultEqualsZero()
    {
        var d = default(EtcdKeyVersion);

        Assert.Equal(EtcdKeyVersion.Zero, d);
    ...
    {
        var k = EtcdKeyVersion.From(v);

        Assert.Equal(v, k.Value);
        Assert.Equal(v == 0L, k.IsZero);
        Assert.Equal(v != 0L, k.IsNonZero);
    }

    [Fact]
    public void FromNegativeThrows()
        => Assert.Throws<ArgumentOutOfRangeException>(()
            => EtcdKeyVersion.From(-1L));

    [Fact]
    public void MaxValueIsInt64Max()
        => Assert.Equal(Int64.MaxValue, EtcdKeyVersion.MaxValue.Value);

    [Fact]
    public void MinValueEqualsZero()
        => Assert.Equal(EtcdKeyVersion.Zero, EtcdKeyVersion.MinValue);

    [Fact]
    public void OperatorsWork()
    {
        var a = EtcdKeyVersion.From(1L);
        var b = EtcdKeyVersion.From(2L);
...
#pragma warning restore CA1859 // Use concrete types when possible for improved performance
        ...
        Assert.Throws<ArgumentException>(()
            => cmp.CompareTo("Definitely not an EtcdKeyVersion"));
    }
    ...
    public void SortingUsesCompareTo()
    {
        var arr = new[]
        {
            EtcdKeyVersion.From(5L),
            EtcdKeyVersion.Zero,
            EtcdKeyVersion.From(2L)
        };
    ...
    public void ToStringFormatsCorrectly(Int64 v, String expected)
        => Assert.Equal(expected, EtcdKeyVersion.From(v).ToString());
}
